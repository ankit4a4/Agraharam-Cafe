...
const HeroHome = () => {
  ...
  useEffect(() => {
    ...
    AOS.init({
      duration: 800,
      once: true, // animate only once for smoother experience
      mirror: false,
      disable: false, // keep enabled but simplified below
      easing: "ease-out",
    });

    // For Safari, limit animations for stability
    if (isSafari) {
      document.querySelectorAll("[data-aos]").forEach((el) => {
        el.removeAttribute("data-aos"); // removes AOS animation
        el.style.opacity = 1; // keeps visible
  ...
  return (
    <section className="relative md:py-32 py-20 flex items-center justify-center overflow-hidden">
      ...
      {/* Decorative Elements */}
      <div className="absolute top-10 left-10 w-16 sm:w-20 md:w-24 h-16 sm:h-20 md:h-24 border-4 border-[#306c36] opacity-30 rounded-full animate-pulse will-change-transform"></div>
      <div className="absolute bottom-10 right-10 w-20 sm:w-28 md:w-32 h-20 sm:h-28 md:h-32 border-4 border-[#306c36] opacity-20 rounded-full animate-ping will-change-transform"></div>

      {/* Content */}
      <div className="relative z-10 text-center max-w-4xl mx-auto px-4 mt-10 text-white">
        {/* Tagline */}
        <div data-aos="fade" className="mb-6">
        ...
        >
          Experience the authentic flavors of South India in a traditional setting
          that transports you to the heart of Karnataka
        </p>
      </div>
    </section>
  );
